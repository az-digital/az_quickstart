 ...
 */
abstract class SortWidgetBase extends AzExposedFiltersWidgetBase implements AzExposedFiltersWidgetInterface {
  ...
  protected $sortElements = [
    'sort_az_exposed_filters_combine',
    'sort_by',
    'sort_order',
  ];
...
Title Desc|Z -> A</pre> Leave the replacement text blank to remove an option altogether. The order the options appear will be changed to match the order of options in this field.'),
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[az_exposed_filters][sort][configuration][advanced][combine]"]' => ['checked' => TRUE],
        ],
      ],
    ];
    ...
    $form['advanced']['reset_label'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[az_exposed_filters][sort][configuration][advanced][reset]"]' => ['checked' => TRUE],
        ],
        'required' => [
          ':input[name="exposed_form_options[az_exposed_filters][sort][configuration][advanced][reset]"]' => ['checked' => TRUE],
        ],
    ...
    $form['advanced']['collapsible_label'] = [
      ...
      '#states' => [
        'visible' => [
          ':input[name="exposed_form_options[az_exposed_filters][sort][configuration][advanced][collapsible]"]' => ['checked' => TRUE],
        ],
        'required' => [
          ':input[name="exposed_form_options[az_exposed_filters][sort][configuration][advanced][collapsible]"]' => ['checked' => TRUE],
        ],
  ...
  public function exposedFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    if ($this->configuration['advanced']['combine'] && !empty($form['sort_order'])) {
      ...
      if (!empty($this->configuration['advanced']['combine_rewrite'])) {
        $options = AzExposedFiltersHelper::rewriteOptions($options, $this->configuration['advanced']['combine_rewrite'], TRUE);
      ...
      $form['sort_az_exposed_filters_combine'] = [
        '#type' => 'select',
        '#options' => $options,
      ...
      $form['#info']['sort-sort_az_exposed_filters_combine'] = [
        'value' => 'sort_az_exposed_filters_combine',
      ];
    ...
    // details element.
    if ($is_collapsible) {
      $form['az_exposed_filters_sort_options'] = [
        '#type' => 'details',
      ...
      if ($is_secondary) {
        ...
        $form['az_exposed_filters_sort_options']['#group'] = 'secondary';
      }
    }
    ...
    foreach ($this->sortElements as $element) {
      ...
      if ($is_collapsible) {
        $this->addElementToGroup($form, $form_state, $element, 'az_exposed_filters_sort_options');
      }
      // Move secondary elements.
      elseif ($is_secondary) {
  ...
  /**
   * Unpacks sort_by and sort_order from the sort_az_exposed_filters_combine element.
   *
   * @param array $form
  ...
  public function sortCombineSubmitForm(array $form, FormStateInterface $form_state) {
    $sort_by = $sort_order = '';
    $combined = $form_state->getValue('sort_az_exposed_filters_combine');
    if (!empty($combined)) {
