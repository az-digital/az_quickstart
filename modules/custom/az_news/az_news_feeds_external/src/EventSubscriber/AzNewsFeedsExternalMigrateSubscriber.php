...
namespace Drupal\az_news_feeds_external\EventSubscriber;
 ...
 */
class AzNewsFeedsExternalMigrateSubscriber implements EventSubscriberInterface {
  ...
  public function onPreImport(MigrateImportEvent $event) {
    ...
    $migration = $event->getMigration();
    if ($migration->id() === 'az_news_feeds_external_stories') {
      $az_news_feeds_external_config = $this->configFactory->getEditable('az_news_feeds_external.settings');
      $selected_terms = $az_news_feeds_external_config->get('uarizona_news_terms');
