...
namespace Drupal\az_opportunity_trellis\Plugin\migrate\source;
...
/**
 * Source plugin for retrieving data via Trellis opportunities.
 */
#[MigrateSource('az_trellis_opportunities_api')]
class AZTrellisOpportunitySource extends SourcePluginBase {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, MigrationInterface $migration) {
    ...
    $this->trellisHelper = \Drupal::service('az_opportunity_trellis.trellis_helper');
  ...
  protected function initializeIterator() {
    // Fetch the opportunities via trellis API.
    $items = $this->trellisHelper->getOpportunities($this->trellisIds);
    $results = [];
    $fields = [];
