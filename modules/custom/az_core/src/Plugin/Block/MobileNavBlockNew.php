 ...
 */
#[Block(
  id: "mobile_nav_block_new",
  admin_label: new TranslatableMarkup("Mobile Nav Block New"),
  category: new TranslatableMarkup("Menus")
)]

class MobileNavBlockNew extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $build['az_mobile_nav_menu_new'] = [
      '#type' => 'html_tag',
      '#tag' => 'div',
      '#attributes' => [
        'id' => 'az_mobile_nav_menu_new',
        'class' => ['mx-2'],
      ],
      '#attached' => [
        'library' => [
    ...
    $build['az_mobile_nav_menu_new']['back'] = [];
    $build['az_mobile_nav_menu_new']['heading'] = [
      '#type' => 'html_tag',
      '#tag' => 'h2',
      '#attributes' => [
        'class' => [
    ...
    $build['az_mobile_nav_menu_new']['menu_links'] = [
      '#type' => 'html_tag',
      '#tag' => 'ul',
      '#attributes' => [
        'id' => 'menu_links',
    ...
    if (empty($tree)) {
      $tree = $this->tree;
      $isMainMenu = TRUE;
      $build['az_mobile_nav_menu_new']['heading']['#attributes']['class'][] = 'pl-3';
      $build['az_mobile_nav_menu_new']['heading']['selected_page'] = [
        '#type' => 'html_tag',
        '#tag' => 'span',
      ...
      $isMainMenu = FALSE;
      $build['az_mobile_nav_menu_new']['heading']['#attributes']['class'][] = 'pl-4';
      ...
      if ($rootElement->link->getRouteName() === '<button>') {
        ...
        if ($rootElement->link->getRouteName() === $this->routeMatch->getRouteName() && $rootElement->link->getRouteParameters() === $this->routeMatch->getRawParameters()->all()) {
          $build['az_mobile_nav_menu_new']['heading']['#attributes']['class'][] = 'az-mobile-nav-current';
        }
      }
    ...
    foreach ($tree as $item) {
      ...
      $build['az_mobile_nav_menu_new']['menu_links'][$item->link->getPluginId()] = [
        '#type' => 'html_tag',
        '#tag' => 'li',
        '#attributes' => [
          'class' => [
