document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.az-gallery-modal').forEach(modal => {
    ...
    document.querySelectorAll('.az-gallery-open-modal[data-bs-target="#' + modal.id + '"]').forEach(thumbnail => {
    ...
    modal.addEventListener('shown.bs.modal', () => {
      if (requestedSlideIndex !== null) {
        ...
        const indicator = carousel.querySelector('.carousel-indicators [data-bs-slide-to="' + requestedSlideIndex + '"]');
        if (indicator) {
          indicator.click();
        }
